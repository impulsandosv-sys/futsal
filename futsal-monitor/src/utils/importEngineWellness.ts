...
export function detectarTipoCuestionario(headers: string[]): TipoCuestionarioWellness | null {
  const tieneDiario = Object.keys(DAILY_WELLNESS_FIELDS).every((k) => headers.includes(k))
  const tieneSemanal = Object.keys(WEEKLY_WELLNESS_FIELDS).every((k) => headers.includes(k))
...
export function procesarFilaWellness(
  ...
  for (const [header, tipoEscala] of Object.entries(WEEKLY_WELLNESS_FIELDS)) {
    const original = parsearNumeroRaw(row[header])
